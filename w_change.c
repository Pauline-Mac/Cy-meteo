...
pArbre creerArbre(int id, float angle, float vitesse, float NScoor, float OEcoor)
{
	...
	// angle --> valeur de x2 : a->OEcoor + a->vitesse*cos(a->angle)
	// vitesse --> valeur de y2 : a->NScoor + a->vitesse*sin(a->angle)
	pnew->counter = 1;
	pnew->id = id;
    pnew->angle = OEcoor + vitesse*cos(angle);
    pnew->vitesse = NScoor +vitesse*sin(angle);
    pnew->NScoor = NScoor;
    pnew->OEcoor = OEcoor;
	pnew->fg = NULL;
	pnew->fd = NULL;
...
void traiter(pArbre a, FILE* outputfile)
{
	...
	fprintf(outputfile, "%0.5f %0.5f %0.5f %0.5f\n", a->OEcoor, a->NScoor, a->angle/a->counter, a->vitesse/a->counter);
}
